...
.resolve_spectral_panel_configuration <- function(cytometer = "aurora", configuration = NULL) {
    ...
    normalized_ids <- .normalize_cytometer_token(configs$id)
    if (key %in% normalized_ids) {
        return(configs$id[match(key, normalized_ids)])
    }
...
.spectral_library_path <- function(cytometer) {
    ...
    path <- .openpanel_extdata_file(file_map[[id]])
...
.read_spectral_library_matrix <- function(cytometer, normalize = TRUE) {
    ...
    if (exists(cache_key, envir = .openpanel_cache)) {
        return(get(cache_key, envir = .openpanel_cache))
    }
    ...
    assign(cache_key, out, envir = .openpanel_cache)
    out
}
...
.match_spectral_fluorophores <- function(library_fluors, requested_fluors, strict = FALSE) {
    ...
    dict_file <- .openpanel_extdata_file("fluorophore_dictionary.csv")
...
.compare_control_spectra <- function(M,
    ...
    library_cytometer <- attr(library, "cytometer")
    ...
    rownames(out) <- NULL
    attr(out, "cytometer") <- library_cytometer
