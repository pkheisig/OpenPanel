...
test('uses bundled metadata and supported cytometer naming schemes', () => {
...
test('accepts legacy scalar array wrappers for cytometer names', () => {
  assert.equal(detectorLaserMeta('UV', ['xenith']).wavelength, '349 nm')
...
test('detector fallbacks and wavelength parsing cover nonstandard panels', () => {
  ...
  assert.equal(detectorAxisDisplayWidth(25, 'conventional'), 1442)
  assert.equal(detectorAxisDisplayWidth(14, 'spectral'), null)
  assert.equal(detectorAxisDisplayWidth(100, 'conventional'), 1800)
  ...
  assert.equal(detectorLaserKey({ detector: 'FL1-A', laser: 'DUV' }), 'DeepUV')
  assert.equal(detectorLaserKey({ detector: 'FL1-A', laser: 'Other' }), 'Other')
  assert.equal(detectorLaserKey('IR1-A'), 'IR')
  assert.equal(mapDetectorToEmission('Filter - 525/20-A'), 525)
  assert.equal(mapDetectorToEmission('UV1 (375)-A'), 375)
  assert.equal(mapDetectorToEmission('488CH4-A'), 500)
  assert.equal(mapDetectorToEmission('unknown'), 0)
  assert.equal(detectorLaserMeta('UV', null).wavelength, '355 nm')
})

test('visible and infrared wavelengths map to bounded RGB colors', () => {
  for (const wavelength of [349, 350, 420, 440, 490, 510, 580, 645, 700, 780, 781]) {
    assert.match(wavelengthToColor(wavelength), /^rgb\(\d+, \d+, \d+\)$/)
  }
  const colors = detectorSpectralColors([
