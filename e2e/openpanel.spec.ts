...
test('selects the instrument and configuration before opening a clean workspace', async ({ page }) => {
  ...
  await page.getByRole('dialog', { name: 'OMIP Library' }).getByRole('button', { name: 'Preview OMIP-111' }).click()
  ...
  await expect(page.getByRole('combobox', { name: 'Marker 1 name' })).toContainText('CD44')
  ...
  await expect(savedCard.getByRole('img', { name: 'Saved panel spectrum preview' }).locator('path')).toHaveCount(1)
  await expect(savedCard.locator('.panel-preview-complexity')).toHaveText('1.00')
