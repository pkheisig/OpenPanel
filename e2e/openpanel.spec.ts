...
test('selects the instrument and configuration before opening a clean workspace', async ({ page }) => {
  ...
  const omipWarning = page.getByRole('alertdialog', { name: 'Apply this panel anyway?' })
  await expect(omipWarning).toContainText('OMIP-111 was designed for Sony ID7000')
  await expect(omipWarning).toContainText('This project uses Sony ID7000 · ID7000 4L: V/B/YG/R')
  ...
  await page.getByRole('button', { name: 'Apply to panel' }).click()
  await page.getByRole('alertdialog', { name: 'Apply this panel anyway?' })
    .getByRole('button', { name: 'Apply anyway' }).click()
...
test('completes a panel through the staged marker wizard', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Apply to panel' })).toBeEnabled()
  await page.getByRole('button', { name: 'Back to OMIP Library' }).click()
