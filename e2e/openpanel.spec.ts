...
test('resizes the spectrum plot from either edge', async ({ page }) => {
  ...
  const expandedViewBox = String(await spectrum.getAttribute('viewBox')).split(' ').map(Number)
  expect(expandedViewBox[2]).toBeGreaterThan(initialViewBox[2] + 40)
  await page.mouse.up()
  ...
  const reducedViewBox = String(await spectrum.getAttribute('viewBox')).split(' ').map(Number)
  expect(reducedViewBox[2]).toBeLessThan(expandedViewBox[2] - 30)
  await page.mouse.up()
...
test('selects the instrument and configuration before opening a clean workspace', async ({ page }) => {
  ...
  expect(reducedBox!.height).toBeCloseTo(baseBox!.height, 0)
  const reducedViewBox = String(await spectrum.getAttribute('viewBox')).split(' ').map(Number)
  expect(Math.abs(reducedViewBox[2] - viewBoxWidth * 0.875)).toBeLessThanOrEqual(1)
  expect(reducedViewBox[3]).toBe(viewBoxHeight)
  ...
  expect(expandedBox!.height).toBeCloseTo(baseBox!.height, 0)
  const expandedViewBox = String(await spectrum.getAttribute('viewBox')).split(' ').map(Number)
  expect(Math.abs(expandedViewBox[2] - viewBoxWidth * 1.125)).toBeLessThanOrEqual(1)
  expect(expandedViewBox[3]).toBe(viewBoxHeight)
  await selectFluorophore(page, 0, 'Alexa Fluor 488')
  await page.waitForTimeout(650)
  await page.getByRole('button', { name: 'Decrease plot size' }).click()
...
test('completes a panel through the staged marker wizard', async ({ page }) => {
  ...
  await expect(conventionalPreview.locator('.omip-library-table tbody tr')).toHaveCount(27)
  await expect(page.getByRole('button', { name: 'Create panel from OMIP' })).toBeEnabled()
